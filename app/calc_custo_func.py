...
if st.button('Calcular'):
    ...
    st.write(f'Férias (1/12): R$ {ferias:.2f}')
    st.write(f'13º Salário (1/12): R$ {decimo_terceiro:.2f}')
    st.write(f'1/3 Férias: R$ {terco_ferias:.2f}')
    st.write(f'FGTS sobre Férias e 13º: R$ {fgts_ferias_13:.2f}')
    st.write(f'Vale Transporte (6%): R$ {vt:.2f}')
    st.write(f'Vale Refeição: R$ {vr:.2f}')
    
    st.subheader('Resumo')
    st.write(f'Custo Total Mensal: R$ {custo_total:.2f}')
    st.write(f'Salário Líquido do Funcionário: R$ {salario_liquido:.2f}')